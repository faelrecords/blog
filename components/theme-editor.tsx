...
export function ThemeEditor({ initialSettings, initialBlocks }: { initialSettings: Record<string, string>; initialBlocks: InitialBlock[] }) {
  ...
  return <div className="visual-theme-builder">
    <header className="builder-toolbar">
      <div className="builder-title"><strong>Editor do tema</strong><span className="theme-save-state"><span className={dirty ? "dirty-dot" : "saved-dot"}/>{dirty ? "Alterações pendentes" : "Tudo publicado"}</span></div>
      <div className="device-switch" aria-label="Tamanho da prévia">{(["desktop", "tablet", "mobile"] as Device[]).map((item) => <button type="button" key={item} className={device === item ? "active" : ""} onClick={() => setDevice(item)} aria-label={`Prévia ${item}`}>{item === "desktop" ? <Monitor/> : item === "tablet" ? <Tablet/> : <Smartphone/>}</button>)}</div>
      <div className="builder-toolbar-actions"><button className="btn btn-ghost" type="button" onClick={restoreDefaults} title="Restaurar padrão"><RotateCcw size={17}/><span>Restaurar</span></button><button className="btn btn-outline" type="button" disabled={!dirty || saving} onClick={discard}>Descartar</button><button className="btn btn-primary" type="button" disabled={!dirty || saving} onClick={save}><Save size={17}/>{saving ? "Publicando..." : "Publicar"}</button></div>
    </header>

    <div className="builder-three-columns">
      <aside className="builder-side builder-elements">
        <div className="builder-side-head"><strong>Elementos</strong><small>Arraste ou clique para adicionar</small></div>
        <div className="section-library" aria-label="Adicionar seção">{HOME_BLOCK_LIBRARY.map((item) => <button type="button" key={item.type} onClick={() => addBlock(item.type)}><Plus size={16}/><span><strong>{item.title}</strong><small>{item.description}</small></span></button>)}</div>
        <div className="builder-divider"/>
        <div className="section-canvas"><div className="section-canvas-title"><strong>Estrutura</strong><small>{blocks.length} seções</small></div>{blocks.length ? blocks.map((block, index) => <div draggable className={`section-row ${selectedId === block.id ? "selected" : ""} ${block.enabled ? "" : "disabled"}`} key={block.id} onDragStart={() => setDraggingId(block.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropOn(block.id)}><GripVertical className="section-grip" size={17}/><button type="button" className="section-select" onClick={() => { setSelectedId(block.id); setTab("pagina"); }}><strong>{block.title}</strong><small>{block.enabled ? "Visível" : "Oculta"}</small></button><div className="section-actions"><button type="button" onClick={() => toggle(block.id)} aria-label={block.enabled ? "Ocultar" : "Exibir"}>{block.enabled ? <Eye size={15}/> : <EyeOff size={15}/>}</button><button type="button" onClick={() => move(index, -1)} disabled={index === 0} aria-label="Mover para cima"><ChevronUp size={15}/></button><button type="button" onClick={() => move(index, 1)} disabled={index === blocks.length - 1} aria-label="Mover para baixo"><ChevronDown size={15}/></button><button type="button" onClick={() => duplicate(block)} aria-label="Duplicar"><Copy size={15}/></button><button type="button" className="danger" onClick={() => removeBlock(block)} aria-label="Excluir"><Trash2 size={15}/></button></div></div>) : <div className="section-empty">Adicione a primeira seção.</div>}</div>
      </aside>

      <main className="builder-page-area" aria-label="Prévia da página"><ThemePreview settings={settings} blocks={blocks} device={device}/></main>

      <aside className="builder-side builder-settings">
        <div className="theme-tabs" role="tablist">{([{ id: "pagina", label: "Seção" }, { id: "marca", label: "Marca" }, { id: "cores", label: "Estilo" }] as { id: Tab; label: string }[]).map((item) => <button type="button" key={item.id} role="tab" aria-selected={tab === item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}>{item.label}</button>)}</div>
        <div className="builder-settings-scroll">
          {tab === "pagina" && (selected ? <BlockInspector block={selected} settings={settings} update={(key, value) => updateConfig(selected.id, key, value)}/> : <div className="section-empty"><Settings2 size={22}/> Selecione uma seção na lateral esquerda.</div>)}
          {tab === "marca" && <div className="theme-form-section"><SectionHeading title="Identidade da marca" text="Logo e nome usados no cabeçalho e rodapé."/><div className="field"><label>Nome da marca</label><input className="input" value={settings.logo_text} onChange={(event) => set("logo_text", event.target.value)}/></div><AssetField label="Logo" value={settings.logo_url} alt={brand} accept="image/png,image/jpeg,image/webp" onUpload={(file) => upload(file, "logo_url")} onRemove={() => set("logo_url", "")}/><AssetField label="Favicon" value={settings.favicon_url} alt="Favicon" accept="image/png,image/jpeg,image/webp" compact onUpload={(file) => upload(file, "favicon_url")} onRemove={() => set("favicon_url", "")}/></div>}
          {tab === "cores" && <div className="theme-form-section"><SectionHeading title="Cores e tipografia" text="Ajustes globais aplicados em todo o blog."/><div className="theme-color-grid">{[["Cor principal", "primary_color"], ["Cor de apoio", "accent_color"], ["Fundo", "background_color"], ["Texto", "text_color"]].map(([label, key]) => <div className="field" key={key}><label>{label}</label><div className="color-field"><input aria-label={`Selecionar ${label.toLowerCase()}`} type="color" value={settings[key]} onChange={(event) => set(key, event.target.value)}/><input className="input" value={settings[key]} onChange={(event) => set(key, event.target.value)}/></div></div>)}</div><FontSelect label="Fonte dos títulos" value={settings.heading_font} onChange={(value) => set("heading_font", value)}/><FontSelect label="Fonte do corpo" value={settings.body_font} onChange={(value) => set("body_font", value)}/></div>}
        </div>
      </aside>
    </div>
    {message && <p className="builder-message" aria-live="polite">{message}</p>}
  </div>;
}
