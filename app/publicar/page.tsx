...
export default async function WriterPage() {
  ...
  if (user.role === "admin") redirect("/admin/artigos");
  const posts = db.prepare(`SELECT p.*,c.name category_name,
    (SELECT note FROM post_reviews r WHERE r.post_id=p.id AND r.action='solicitar_alteracoes' ORDER BY r.id DESC LIMIT 1) review_note
    FROM posts p LEFT JOIN categories c ON c.id=p.category_id WHERE p.author_id=? ORDER BY p.updated_at DESC`).all(user.id) as { id: number; title: string; status: string; category_name: string | null; updated_at: string; review_note: string | null }[];

  return <AdminShell user={user} title="Espaço editorial"><div className="app-content">
    <div className="page-head"><div><h1 className="app-title">Meus artigos</h1><p className="muted">Escreva com tranquilidade. Seus rascunhos são salvos automaticamente.</p></div><Link className="btn btn-primary" href="/publicar/novo">+ Novo artigo</Link></div>
